...
### 6.5 React Components (Figma 디자인 기반)

#### 🎨 Figma 디자인 구조

**레이아웃:**
- **헤더**: 고정 (fixed top)
  - 로고 섹션: "FINE WINE SELECTION" + 서브타이틀
  - 필터 카드: 가로 스크롤 (160px x 80px, 2:1 비율)
- **메인**: 4열 그리드 와인 카드
  - 와인 이미지: **1:1 정사각형 비율** (aspect-ratio: 1)
  - 카드 정보: 와인명, 국가, 와이너리, Vivino 평점, 가격
  - 장바구니 추가 버튼 (카드 하단)
- **푸터**: 고정 (fixed bottom)
  - 검색 버튼 + 장바구니 버튼

**색상:**
- 배경: `#2F161A` → `#1C0E10` (radial gradient)
- 카드: `#f5f0e8` (베이지)
- 텍스트: `#3d2618` (다크 브라운), `#8b6f47` (밝은 브라운)
- 강조: `#d4af37` (골드)
- 버튼: `#6b2c2c` (와인 레드), hover `#8b3a3a`
  ...
  return (
    <div className="min-h-screen relative" style={{ width: '1080px', margin: '0 auto' }}>
      ...
      <div className="relative z-10">
        ...
        {/* Main Content */}
        <main className="pt-[260px] pb-[100px] px-6">
          {children}
        </main>
...
#### src/frontend/components/KioskHeader.tsx
...
const filters: Filter[] = [
  { id: 'all', label: 'ALL', image: 'https://images.unsplash.com/...' },
  { id: 'red', label: 'Red Wine', image: 'https://images.unsplash.com/...' },
  { id: 'white', label: 'White Wine', image: 'https://images.unsplash.com/...' },
  { id: 'rose', label: 'Rosé Wine', image: 'https://images.unsplash.com/...' },
  { id: 'sparkling', label: 'Sparkling Wine', image: 'https://images.unsplash.com/...' },
  { id: 'france', label: 'France', image: 'https://images.unsplash.com/...' },
  { id: 'italy', label: 'Italy', image: 'https://images.unsplash.com/...' },
  { id: 'spain', label: 'Spain', image: 'https://images.unsplash.com/...' },
]
...
#### src/frontend/components/FilterCard.tsx
...
export default function FilterCard({ label, image, active, onClick }: FilterCardProps) {
  return (
    <button
      onClick={onClick}
    ...
    >
      ...
      <div className="absolute inset-0 flex items-center justify-center">
        <span className="text-[#f5f0e8] uppercase tracking-wider px-4 text-center text-sm">
          {label}
        </span>
      </div>
    </button>
...
#### src/frontend/components/KioskFooter.tsx
      ...
      <div className="max-w-[1080px] mx-auto px-6 py-4 relative z-10">
        <div className="flex gap-4">
          ...
          >
            ...
            {cartItemCount > 0 && (
              <>
                <span className="absolute top-2 right-2 bg-[#d4af37] text-[#2b1810] text-xs w-6 h-6 rounded-full flex items-center justify-center font-bold">
                  {cartItemCount}
                </span>
...
**디자인 요구사항 (Figma 참조):**
- 배경: 베이지 (#f5f0e8)
- **이미지: 1:1 정사각형 비율** (aspect-ratio: 1)
- 정보 순서:
  1. 와인명 (2줄 말줄임)
  2. Vivino 평점 (골드 별)
  3. 국가명, 와이너리
...
export default function WineCard({ wine, onAddToCart }: Props) {
  return (
    <div className="bg-[#f5f0e8] rounded-lg overflow-hidden flex flex-col h-full">
      ...
      <div className="p-4 flex-1 flex flex-col">
        ...
        <div className="flex-1 flex flex-col">
          {/* 와인명 */}
          <h3 className="text-[#3d2618] font-medium mb-1 line-clamp-2">
            {wine.title}
          </h3>
          ...
          {wine.points && (
            <div className="flex items-center gap-1 mb-2">
              ...
              <span className="text-[#3d2618] text-sm font-medium">
          ...
          <div className="text-xs text-[#8b6f47] mb-2">
            {wine.country && <div>{wine.country}</div>}
            {wine.winery && <div>{wine.winery}</div>}
          </div>
          ...
          {/* 가격 */}
          {wine.price && (
            <div className="text-[#3d2618] font-bold mt-auto mb-3">
      ...
      <div className="px-3 pb-3">
        <button
          ...
          className="w-full bg-[#6b2c2c] text-[#f5f0e8] py-3 rounded-lg hover:bg-[#8b3a3a] transition-colors flex items-center justify-center gap-2 font-medium"
        >
...
### 6.6 CSS 스타일 (Figma 디자인 시스템)

#### 🎨 Figma 디자인 시스템 색상 팔레트

**배경:**
- Dark gradient: `#2F161A` → `#1C0E10` (radial)
- Noise overlay (투명도 15-35%)

**카드 & 텍스트:**
- 카드 배경: `#f5f0e8` (베이지)
- 다크 텍스트: `#3d2618` (다크 브라운)
- 밝은 텍스트: `#8b6f47` (밝은 브라운)
- 헤더 텍스트: `#f5f0e8` (밝은 베이지)
- 서브 텍스트: `#a08967` (회갈색)
...
:root {
  /* 색상 */
  --color-dark-bg-start: #2F161A;
  --color-dark-bg-end: #1C0E10;
  --color-card-bg: #f5f0e8;
  --color-dark-text: #3d2618;
  --color-light-text: #8b6f47;
  --color-header-text: #f5f0e8;
  --color-sub-text: #a08967;
  --color-gold: #d4af37;
  --color-wine-red: #6b2c2c;
  --color-wine-red-hover: #8b3a3a;
  --color-dark-brown: #3d2618;
  --color-dark-brown-hover: #4a3225;
